      ...
      keys:
        '`ENTITY_ID`':
          ...
          keys:
            ...
            co_threshold:
              description: Only for `sensor` entities with `device_class` `carbon_monoxide` or `carbon_monoxide` in `entity_id`. Used as the threshold value once HomeKit will warn/notify the user.
              required: false
              type: integer
              default: 25
            co2_threshold:
              description: Only for `sensor` entities with `device_class` `carbon_dioxide` or `carbon_dioxide` in `entity_id`. Used as the threshold value once HomeKit will warn/notify the user.
              required: false
              type: integer
              default: 1000
    devices:
